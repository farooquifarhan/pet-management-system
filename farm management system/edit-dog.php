...
<?php 
 if(!$_GET['id'] OR empty($_GET['id']) OR $_GET['id'] == '')
 {
 	...
 	$dogno = $weight = $gender = $remark = $arr = $bname = $b_id = $health = $img = "";
 	$id = (int)$_GET['id'];
...
<div class="w3-container" style="padding-top:22px">
 <div class="w3-row">
     ...
     <?php
      if(isset($_POST['submit']))
      {
      	...
      	$update_query = $db->query("UPDATE dogs SET dogno = '$n_dogno',weight = '$n_weight',arrived = '$n_arrived', breed_id = '$n_breed', remark = '$n_remark',health_status = '$n_status' WHERE id = '$n_id' ");

      	if($update_query){?>
      	<div class="alert alert-success alert-dismissable">
            <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
             <strong>Dog details successfully update <i class="fa fa-check"></i></strong>
        </div>
       <?php
      	}else{ ?>
          <div class="alert alert-danger alert-dismissable">
            <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
             <strong>Error updating dog data. Please try again <i class="fa fa-times"></i></strong>
        </div>
      	<?php
      }
     ...
     ?>
	 	...
	 	<form method="post">
	 		<div class="form-group">
	 			...
	 			<input type="text" name="dogno" class="form-control" value="<?php echo $dogno; ?>">
	 		</div>
 ...
 <div class="col-md-4 col-md-offset-2">
 	...
 	<a class="btn btn-danger btn-md" onclick="return confirm('Continue delete dog ?')" href="delete.php?id=<?php echo $id ?>"><i class="fa fa-trash"></i> Delete Dog</a>
 </div>
</div>
</div>
</div>
